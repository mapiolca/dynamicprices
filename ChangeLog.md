...
- Alignement du dictionnaire des catégories commerciales sur le partage Multicompany des produits, afin que les catégories suivent les produits partagés entre entités. / Aligned the commercial categories dictionary with product Multicompany sharing so categories follow products shared between entities.
- Correction de la déclaration des dictionnaires DynamicPrices avec des clés internes cohérentes et des noms de tables non préfixés, afin d'éviter les erreurs d'index dans l'administration native des dictionnaires. / Fixed the DynamicPrices dictionary declaration with consistent internal keys and unprefixed table names to avoid index errors in the native dictionary administration.
- Application du partage Multicompany aux lignes des dictionnaires DynamicPrices, sans créer de déclaration de dictionnaire différente par entité. / Applied Multicompany sharing to DynamicPrices dictionary rows without creating a different dictionary declaration per entity.
