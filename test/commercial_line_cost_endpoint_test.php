...
if ($scenario === '') {
	...
	foreach ($cases as $case => $expectedStatus) {
		...
		if (strpos($case, 'trigger_') === 0) {
			$denied = in_array($case, array('trigger_no_document_right', 'trigger_document_denied', 'trigger_thirdparty_denied', 'trigger_price_error', 'trigger_document_entity'), true);
			$unchanged = $denied || in_array($case, array('trigger_update', 'trigger_never'), true);
			if (($payload['result'] < 0) !== $denied || (float) $payload['cost'] !== ($unchanged ? 70.0 : 12.5)
...
class User
{
	...
	public function hasRight($module, ...$permissions) {
		...
		if ($scenario === 'no_document_right' && $module === 'propal') { return false; }
