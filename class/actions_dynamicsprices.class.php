...
class ActionsDynamicsPrices extends CommonHookActions
{
	...
	public function doActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($this->isHookContext($parameters, 'productcard')) {
	...
	/**
	 * Display DynamicPrices cost block on product card.
	 *
	 * @param array<string,mixed> $parameters Hook parameters
	 * @param CommonObject $object Current object
	 * @param string $action Current action
	...
	public function formObjectOptions($parameters, &$object, &$action, $hookmanager)
	{
		...
		$this->resprints .= $this->renderProductCostBlock($object, $record);
	...
	private function doProductCostActions($parameters, &$object, &$action, $hookmanager)
	{
		...
		if ($action === 'dynamicsprices_preview_cost') {
			...
			$calculation = $service->calculateProductCost($productId, array('entity' => $entity, 'calculation_context' => 'productcard_preview'));
		...
		$result = $service->recalculateProductCost($productId, $user, array('entity' => $entity, 'calculation_context' => 'productcard'));
		if ($result < 0) {
	...
	public function formConfirm($parameters, &$object, &$action, $hookmanager)
	{
		...
		$html .= '<td class="center maxwidth100">'.$langs->trans('LMDB_SupplierRef').'</td>';
		$html .= '<td class="center maxwidth50">'.$langs->trans('QtyMin').'</td>';
		$html .= '<td class="center maxwidth100">'.$langs->trans('LMDB_QuantityPackaging').'</td>';
		$html .= '<td class="center maxwidth50">'.$langs->trans('VATRate').'</td>';
		$html .= '<td class="center maxwidth50">'.$langs->trans('LMDB_CurrentUnitPriceHT').'</td>';
		$html .= '<td class="center maxwidth50">'.$langs->trans('LMDB_ProposedUnitPriceHT').'</td>';
		$html .= '<td class="center maxwidth50">'.$langs->trans('LMDB_PriceDeltaHT').'</td>';
		$html .= '<td class="center maxwidth50">'.$langs->trans('LMDB_PriceDirection').'</td>';
		$html .= '<td class="center maxwidth50">'.$langs->trans('Discount').' (%)</td>';
		$html .= '</tr>';
		...
		foreach ($displayDifferences as $lineId => $diff) {
			...
			$html .= '<td class="right"><input class="center width50" type="text" name="dynamicsprices_data['.$lineId.'][unitprice]" value="'.dol_escape_htmltag((string) $diff['new_unitprice']).'"></td>';
			...
			$html .= '<td class="right"><input class="center width50" type="text" name="dynamicsprices_data['.$lineId.'][discount]" value="'.dol_escape_htmltag((string) $diff['discount']).'" placeholder="%"></td>';
	 ...
	 * @return string
	 */
	private function renderProductCostBlock($product, $record)
	{
		global $langs, $user;
