...
class ActionsDynamicsPrices extends CommonHookActions
{
	...
	public function formConfirm($parameters, &$object, &$action, $hookmanager)
	{
		...
		foreach ($displayDifferences as $lineId => $diff) {
			...
			$html .= '<td class="right"><input class="right width50" type="text" name="dynamicsprices_data['.$lineId.'][discount]" value="'.dol_escape_htmltag((string) $diff['discount']).'" placeholder="%"></td>';
